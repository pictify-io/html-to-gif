...
class WebAnimationRecorder {
  constructor(url, html, width, height, framesPerSecond, selector, browser) {
  ...
  async createGif(pics) {
    // await Promise.all(pics.map((pic) => new Promise(async (resolve, reject) => {
    //   const resizedPic = await sharp(pic)
    //     .resize(Math.round(this.maxBounds.width / 2), Math.round(this.maxBounds.height / 2))
...
async function createGif({
  ...
  console.log(`Setup browser in ${t2 - t1} ms`);
  await recorder.gatherAnimations();
  const t3 = performance.now();
  console.log(`Gathered animations in ${t3 - t2} ms`);
  const { uploadPromise } = await recorder.prepareRecording();
  const t4 = performance.now();
  console.log(`Prepared recording in ${t4 - t3} ms`);
  await recorder.record();
  const t5 = performance.now();
  console.log(`Recorded in ${t5 - t4} ms`);
